...
// Commerce API - V1 회원 요청 규약 테스트 (네트워크 호출 없음)
//   1) 로그인:      POST   users/login       (login_id / password / corporate_type)
//   2) 세션 조회:   GET    users/session     (Bootpay-User-JWT)
//   3) 로그아웃:    DELETE users/session     (Bootpay-User-JWT)
//   4) 회원가입:    POST   users/join        (null/undefined 값은 전송하지 않는다)
//   5) 중복 확인:   GET    users/join/{type}?pk={pk}
//   ⚠️ 단수 user/... 는 commerce-api v1 에 존재하지 않는 죽은 경로다. 전부 복수 users/... 여야 한다.
//   ⚠️ 로그인은 POST /v1/users/login 이다. POST /v1/users/session 은 라우트만 있고 create 액션이 없다.
...
(async () => {
    ...
    assert.strictEqual(requests[0].url, 'https://dev-api.bootapi.com/v1/users/login');
    ...
    assert.strictEqual(requests[2].url, 'https://dev-api.bootapi.com/v1/users/session');
    ...
    // 5) 회원 로그아웃 — DELETE users/session
    await commerce.user.userLogout('USER_JWT', 'logout-key');
    assert.strictEqual(requests[4].method.toLowerCase(), 'delete');
    assert.strictEqual(requests[4].url, 'https://dev-api.bootapi.com/v1/users/session');
    ...
    assert.strictEqual(requests[5].url, 'https://dev-api.bootapi.com/v1/users/join');
    ...
    assert.strictEqual(
        requests[7].url,
        'https://dev-api.bootapi.com/v1/users/join/group-business-number-exist?pk=123-45-67890'
    );
    assert.strictEqual(header(requests[7], 'Idempotency-Key'), 'check-key');

    // 8) 간편 헬퍼(login / checkExist)도 같은 경로를 쓴다
    ...
    // 9) uid-exist 전용형 — email/id/phone/group-business-number 와 같은 패턴
    await commerce.user.uidExist('external_uid_1234', 'uid-key');
    assert.strictEqual(requests[10].method.toLowerCase(), 'get');
    assert.strictEqual(
        requests[10].url,
        'https://dev-api.bootapi.com/v1/users/join/uid-exist?pk=external_uid_1234'
    );
    assert.strictEqual(header(requests[10], 'Idempotency-Key'), 'uid-key');
    assert.strictEqual(header(requests[10], 'BOOTPAY-ROLE'), 'user');

    console.log('commerce v1 user: users/login, users/session, users/join endpoints + JWT header');
})().catch((error) => {
    console.error(error);
    process.exit(1);
});
