...
//   1) 목록: GET products (page/limit 기본값 1/20, category_id/sort/keyword 는 지정된 것만)
...
(async () => {
    ...
    assert.strictEqual(requests[2].url, 'https://dev-api.bootapi.com/v1/products/PRODUCT_ID');
    assert.ok(header(requests[2], 'Idempotency-Key'), 'Idempotency-Key header is required');
    assert.strictEqual(header(requests[2], 'Bootpay-User-JWT'), undefined);

    // 4) 상품 상세 — 회원 JWT 지정
    await commerce.product.productDetail('PRODUCT_ID', 'USER_JWT', 'detail-key');
    assert.strictEqual(header(requests[3], 'Bootpay-User-JWT'), 'USER_JWT');
    assert.strictEqual(header(requests[3], 'Idempotency-Key'), 'detail-key');

    // 5) 기존 list() / detail() 은 그대로 유지된다 (기본 page/limit 없음)
    await commerce.product.list({ page: 1, limit: 10, type: 0 });
    assert.strictEqual(requests[4].url, 'https://dev-api.bootapi.com/v1/products?page=1&limit=10&type=0');

    await commerce.product.detail('PRODUCT_ID');
    assert.strictEqual(requests[5].url, 'https://dev-api.bootapi.com/v1/products/PRODUCT_ID');
