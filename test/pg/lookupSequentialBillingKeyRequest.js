...
(async () => {
    ...
    await Bootpay.lookupSequentialBillingKey('widget_key_1', 'billing_key_1');
    assert.strictEqual(requests[0].method.toLowerCase(), 'get');
    assert.strictEqual(
        requests[0].url,
        'https://dev-api.bootpay.co.kr/v2/subscribe/sequential_billing_key/billing_key_1?widget_key=widget_key_1'
    );

    console.log('pg lookupSequentialBillingKey: billing_key in path, widget_key in query');
})().catch((error) => {
    console.error(error);
    process.exit(1);
});
